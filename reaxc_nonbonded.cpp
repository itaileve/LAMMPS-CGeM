                         ...
                         reax_list **lists, output_controls * /*out_control*/ )
{
  ...
  for( i = 0; i < natoms; ++i ) {
    ...
    if (system->my_atoms[i].type < 0) continue;
    if(strcmp(system->reax_param.sbp[system->my_atoms[i].type].name,"EL")==0)continue; // CGeM
    ...
    for( pj = start_i; pj < end_i; ++pj ) {
      ...
      if (system->my_atoms[j].type < 0) continue;
      if(strcmp(system->reax_param.sbp[system->my_atoms[j].type].name,"EL")==0)continue; // CGeM
                                   ...
                                   output_controls * /*out_control*/ )
{
  ...
  for( i = 0; i < natoms; ++i ) {
	...
	//e_ele *= system->my_atoms[i].q * system->my_atoms[j].q;
      //printf("type i=%i type j=%i r_vdw=%.8f \n", system->my_atoms[i].type,system->my_atoms[j].type,system->reax_param.tbp[type_i][type_j].r_vdW);
