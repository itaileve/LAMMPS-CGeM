                     ...
                     reax_list **lists, output_controls *out_control )
{
  ...
  for( j = 0; j < natoms; ++j ) {
    ...
    if(strcmp(system->reax_param.sbp[type_j].name,"EL")==0)continue; // CGeM
    ...
    for( pk = start_j; pk < end_j; ++pk ) {
	  ...
	  if(strcmp(system->reax_param.sbp[type_k].name,"EL")==0)continue; // CGeM
	      ...
	      if(strcmp(system->reax_param.sbp[type_i].name,"EL")==0)continue; // CGeM
              r_ij = pbond_ij->d;
		...
		if(strcmp(system->reax_param.sbp[type_l].name,"EL")==0)continue; // CGeM
