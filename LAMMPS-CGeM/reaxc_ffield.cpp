                       ...
                       control_params *control )
{
  ...
  for( i = 0; i < reax->num_atom_types; i++ ) {
    ...
    val = atof(tmp[2]); reax->sbp[i].vdW_flag   = val;  //CGeM
    val = atof(tmp[3]); reax->sbp[i].b_o_131    = val;
    val = atof(tmp[4]); reax->sbp[i].b_o_132    = val;
    val = atof(tmp[5]); reax->sbp[i].b_o_133    = val;
    val = atof(tmp[6]); reax->sbp[i].Chi_CGeM   = val;  //CGeM
    val = atof(tmp[7]); reax->sbp[i].R_CGeM     = val;  //CGeM
    
    //CGeM
    reax->sbp[i].alpha_CGeM=reax->gp.l[22]/(2.0*reax->sbp[i].R_CGeM*reax->sbp[i].R_CGeM);

    if(strcmp(reax->sbp[i].name,"EL") ==0)reax->sbp[i].gamma_CGeM=reax->gp.l[35]/(reax->sbp[i].R_CGeM);
    else reax->sbp[i].gamma_CGeM=reax->gp.l[34]/(reax->sbp[i].R_CGeM);
    //printf("type_i=%d alpha=%.8f Chi=%.8f gamma=%.8f lambda=%.8f omega2=%.8f R=%.8f\n",i,reax->sbp[i].alpha_CGeM,reax->sbp[i].Chi_CGeM,reax->sbp[i].gamma_CGeM,reax->gp.l[22],reax->gp.l[35],reax->sbp[i].R_CGeM);
    ...
    //printf("i=%i C6=%.4f Re=%.4f \n",i,reax->sbp[i].lgcij,reax->sbp[i].lgre);
  ...
  for (i=0; i < reax->num_atom_types; i++)
    for (j=i; j < reax->num_atom_types; j++) {
      ...
      if (reax->sbp[j].vdW_flag == 1 || reax->sbp[i].vdW_flag == 1)
	reax->tbp[j][i].vdW_flag = reax->tbp[i][j].vdW_flag = 1;
      else
	reax->tbp[j][i].vdW_flag = reax->tbp[i][j].vdW_flag = 0;
      ...
      //  sqrt( reax->sbp[i].lgre*reax->sbp[j].lgre );  //CGeM

      reax->tbp[i][j].lgre = reax->tbp[j][i].lgre = 2.0 * 1.0 *
  ...
  for( i = 0; i < reax->num_atom_types; i++ ){
    for( j = 0; j <= i; j++ ){
      reax->tbp[i][j].alpha_CGeM=sqrt(reax->sbp[i].alpha_CGeM*reax->sbp[j].alpha_CGeM/(reax->sbp[i].alpha_CGeM + reax->sbp[j].alpha_CGeM));
      reax->tbp[j][i].alpha_CGeM=reax->tbp[i][j].alpha_CGeM;
    }
  }
  ...
  for( i = 0; i < reax->num_atom_types; i++ ){
    for( j = 0; j <= i; j++ ){
      ...
      if((strcmp(reax->sbp[i].name,"EL")==0 || strcmp(reax->sbp[j].name,"EL") ==0) &&  reax->sbp[i].Chi_CGeM != 0 && reax->sbp[j].Chi_CGeM !=0){
	if(strcmp(reax->sbp[i].name,"EL")!=0){
	  i_core=i;
	  i_shell=j;
	}
	else if(strcmp(reax->sbp[j].name,"EL")!=0){
	  i_core=j;
	  i_shell=i;
	}
	else i_core=i_shell=i;
	//else if(j!=12)i_core=j;
	//
	
	//printf("i=%d j=%d Chi=%.8f",i,j,);
	//Beta[type_i] =            (Beta[type_i] - Coulomb_const/(Tiny)*math.erf(erf_alpha*Tiny))        /(math.exp(-gamma[type_i]*Tiny**2.0));
	//if(i!=12 || j!=12)
	if(strcmp(reax->sbp[i].name,"EL")!=0 || strcmp(reax->sbp[j].name,"EL") !=0)
	  reax->tbp[i][j].beta_CGeM=(reax->sbp[i_core].Chi_CGeM - 14.4/(Tiny)*erf(reax->tbp[i_core][i_shell].alpha_CGeM*Tiny))/(exp(-reax->sbp[i_core].gamma_CGeM*Tiny*Tiny));
	else
	  reax->tbp[i][j].beta_CGeM=(reax->sbp[i_core].Chi_CGeM + 14.4/(Tiny)*erf(reax->tbp[i_core][i_shell].alpha_CGeM*Tiny))/(exp(-reax->sbp[i_core].gamma_CGeM*Tiny*Tiny));
	reax->tbp[j][i].beta_CGeM=reax->tbp[i][j].beta_CGeM;
	//printf("i=%d j=%d Beta=%.4f E_R_0=%.4f 1.0=%.4f Chi=%.8f alpha=%.4f\n",i,j,reax->tbp[i][j].beta_CGeM,14.4/(Tiny)*erf(reax->tbp[i_core][i_shell].alpha_CGeM*Tiny),(exp(-reax->sbp[i_core].gamma_CGeM*Tiny*Tiny)),reax->sbp[i_core].Chi_CGeM,reax->tbp[i_core][i_shell].alpha_CGeM);
	//printf("beta=%.8f\n",reax->tbp[i][j].beta_CGeM);
      }
      else{
	reax->tbp[j][i].beta_CGeM=0.0;
	reax->tbp[i][j].beta_CGeM=0.0;
      }
    }
  }
