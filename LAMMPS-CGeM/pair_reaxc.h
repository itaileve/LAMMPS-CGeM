...
class PairReaxC : public Pair {
 ...
 protected:
  ...
  void get_names(char *, double *&);//electric field
  int nmax;
  void FindBond();
  double memory_usage();
  void CGeM_Minimization(reax_system*,control_params*,
			 simulation_data*,storage*);
  double CGeM_Energy(reax_system*,control_params*,
		     simulation_data*,storage*);
  double CGeM_Forces(reax_system*,control_params*,
		   simulation_data*,storage*);
  double CGeM_dRij( reax_system*,int,int,int,double);
  double d_erf_di( reax_system*,double,double);
  void distance(reax_system*,rvec,rvec, double*, rvec* );
};
