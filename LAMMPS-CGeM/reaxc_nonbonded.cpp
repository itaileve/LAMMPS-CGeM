                         ...
                         reax_list **lists, output_controls * /*out_control*/ )
{
  ...
  //printf("CGeM Energy Init= %.8f\n", Energy);
  //CGeM_Minimization(system,control,data,workspace,lists);
  //Energy = CGeM_Energy(system,control,data,workspace,lists);
  //printf("CGeM Energy Final= %.8f\n", Energy);
  //e_ele=data->my_en.e_ele = 23.06*Energy;
  
  for( i = 0; i < natoms; ++i ) {
    //if (system->my_atoms[i].type < 0) continue;
    if (system->my_atoms[i].type < 0) continue;
    if(strcmp(system->reax_param.sbp[system->my_atoms[i].type].name,"EL")==0 || strcmp(system->reax_param.sbp[system->my_atoms[i].type].name,"DU")==0)continue; // CGeM
    //std::cout<<system->my_atoms[i].type<<std::endl;
    //std::cout<<strcmp(system->reax_param.sbp[system->my_atoms[i].type].name<<std::endl;
    ...
    for( pj = start_i; pj < end_i; ++pj ) {
      ...
      if (system->my_atoms[j].type < 0) continue;
      if(strcmp(system->reax_param.sbp[system->my_atoms[j].type].name,"EL")==0 || strcmp(system->reax_param.sbp[system->my_atoms[j].type].name,"DU")==0)continue; // CGeM
      ...
      if(system->reax_param.gp.vdw_type==1 || system->reax_param.gp.vdw_type==3)
        { // shielding
          ...
          exp2 = twbp->vdW_flag * exp( 0.5 * twbp->alpha * (1.0 - fn13 / twbp->r_vdW) );
	  //exp2=0; //CGeM
        ...
        exp2 = twbp->vdW_flag * exp( 0.5 * twbp->alpha * (1.0 - r_ij / twbp->r_vdW) );
	//exp2=0; //CGeM
        e_vdW = twbp->D * (exp1 - 2.0 * exp2);
        data->my_en.e_vdW += Tap * e_vdW;
                                   ...
                                   output_controls * /*out_control*/ )
{
  ...
  //printf("CGeM Energy Final= %.8f\n", Energy);
  //e_ele = data->my_en.e_ele = 23.06*Energy;
  //system->pair_ptr->ev_tally(i,j,natoms,1,0,e_ele,
  //			     f_tmp,delij[0],delij[1],delij[2]);
  //exit(0);
  //for( i = 0; i < natoms; ++i ) {
    //workspace->f[i][0]-=23.06*system->my_atoms[i].f_CGeM[0];
    //workspace->f[i][1]-=23.06*system->my_atoms[i].f_CGeM[1];
    //workspace->f[i][2]-=23.06*system->my_atoms[i].f_CGeM[2];
  //}
  ...
  for( i = 0; i < natoms; ++i ) {
    ...
    //printf("type i= %.d CGeM_R=%.8f \n", type_i,system->reax_param.sbp[type_i].R_CGeM);
    if (system->my_atoms[i].type < 0) continue;
    if(strcmp(system->reax_param.sbp[system->my_atoms[i].type].name,"EL")==0 || strcmp(system->reax_param.sbp[system->my_atoms[i].type].name,"DU")==0)continue; // CGeM
    //std::cout<<system->my_atoms[i].type<<std::endl;
    start_i = Start_Index(i,far_nbrs);
    ...
    for( pj = start_i; pj < end_i; ++pj ) {
      ...
      if (system->my_atoms[j].type < 0) continue;
      if(strcmp(system->reax_param.sbp[system->my_atoms[j].type].name,"EL")==0 || strcmp(system->reax_param.sbp[system->my_atoms[j].type].name,"DU")==0)continue; // CGeM
      //std::cout<<system->my_atoms[j].type<<std::endl;
      orig_j  = system->my_atoms[j].orig_id;
	...
	//e_ele *= system->my_atoms[i].q * system->my_atoms[j].q;
      //printf("type i=%i type j=%i r_vdw=%.8f e_vdw=%.8f\n", system->my_atoms[i].type,system->my_atoms[j].type,system->reax_param.tbp[type_i][type_j].r_vdW, e_vdW);
	...
	//CEclmb *= system->my_atoms[i].q * system->my_atoms[j].q;
      ...
      if (control->virial == 0) {
        ...
        //rvec_Scale( temp, CEvd + CEclmb, nbr_pj->dvec );
	rvec_Scale( temp, CEvd , nbr_pj->dvec );
  ...
  if(system->reax_param.gp.vdw_type==2 || system->reax_param.gp.vdw_type==3)
    { // inner wall
      ...
      if (control->lgflag) {
        ...
        e_lg = -(twbp->lgcij/( r_ij6 + re6 ));
        lr->e_vdW += Tap * e_lg;
	//printf("C6=%.4f r_ij6=%.4f re6=%.4f r_ij=%.4f E_vdW=%.4f\n",twbp->lgcij,r_ij6,re6,r_ij,e_lg);
        de_lg = -6.0 * e_lg *  r_ij5 / ( r_ij6 + re6 ) ;
